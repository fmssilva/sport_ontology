...
% Define style for ontology axioms
\lstdefinestyle{axiomstyle}{
    backgroundcolor=\color{gray!20},
    commentstyle=\color{green!40!black},
    keywordstyle=\color{blue!80!black},
    identifierstyle=\color{purple!70!black},
    numberstyle=\tiny\color{gray},
    basicstyle=\ttfamily\footnotesize,
    breakatwhitespace=false,
    breaklines=true,
    ...
    frame=lines,
    mathescape=true,
    morekeywords={equiv, subclassOf, subPropertyOf, exists, forall, and, or, not, some, only, min, max, exactly, atMost, atLeast, hasMarketValue, hasPosition, hasAge, hasContract, hasJerseyNumber, hasStartDate, hasEndDate, playsFor, competesIn, participatesIn, belongsTo, hasAuthority, TopPlayer, Player, GoalKeeper, YouthPlayer, SeniorPlayer, Coach, Team, SeniorTeam, ValidContract, Nothing, Date, Integer}
}

% Define style for SPARQL queries
\lstdefinestyle{sparqlstyle}{
    ...
    tabsize=2,
    frame=lines,
    morekeywords={SELECT, WHERE, FILTER, ORDER, BY, DESC, ASC, DISTINCT, LIMIT, OFFSET, OPTIONAL, UNION, GRAPH, FROM, NAMED, PREFIX, BASE, CONSTRUCT, ASK, DESCRIBE, GROUP, HAVING, BIND, VALUES, MINUS, SERVICE, EXISTS, NOT, a, rdfs, rdf, owl, xsd}
}
...
The implementation achieves professional complexity across five major domains: Person hierarchy (25+ classes including Player/Coach/StaffMember specializations), Team classification system (8 classes for age-based and performance-based categories), Organizational structure (6 classes for federation/league/club entities), Competition framework (6 classes for domestic/international competitions), and Contract system (8 classes for duration and purpose-based classifications). This 40+ class structure with 3-4 level hierarchical depth matches Pizza ontology complexity standards while maintaining HermiT reasoning performance under 5 seconds. The complete class taxonomy is shown in Figure~\ref{fig:class-hierarchy}.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{images/Ontology_Class_Hierarchy.png}
...
Performance optimization focused on reducing practical reasoning overhead despite the ontology's inherent ExpTime complexity. Property chains were limited to essential inferences (e.g. playsFor $\circ$ competesIn $\rightarrow$ participatesIn) to minimize computational burden during query evaluation. Class expressions avoided deeply nested quantifications to maintain reasonable reasoning times in practice. The OWL 2 QL profile compatibility for OBDA scenarios provides polynomial-time query answering over data (ABox queries), while full TBox+ABox reasoning remains in ExpTime. SPARQL endpoint configuration includes result set limits and timeout controls for production deployment.
...
\begin{lstlisting}[style=mystyle, caption=Ontology Design Patterns Examples]
-- TopPlayer auto-classification
...
\begin{lstlisting}[style=mystyle, caption=Functional Property Constraint Resolution]
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{images/TopPlayer_Inference.png}
\caption{Automatic TopPlayer Classification Through Semantic Reasoning}
\label{fig:auto-classification}
\end{figure}

\subsection{Technical Challenges}

OWL 2 QL profile compliance presented the most significant technical obstacle during development. Initial ontology versions utilized complex class expressions incompatible with the QL profile's restrictions. Nested quantifications and intricate property restrictions caused query rewriting failures in Ontop, requiring systematic simplification of axiom structures to maintain OBDA compatibility while preserving essential reasoning capabilities in the full ontology.
...
\textbf{H2 Database Integration Issues:} Windows development environments revealed critical configuration challenges. H2 returns fully qualified table names (\texttt{"SPORTS-DB"."PUBLIC"."TEAM"}) while Ontop mappings expected simple names (\texttt{TEAM}), causing relation mismatch errors. The solution required H2 configuration parameters: 
\begin{itemize}
\item \texttt{DATABASE\_TO\_UPPER=true}
\item \texttt{CASE\_INSENSITIVE\_IDENTIFIERS=true}
\end{itemize}
These settings ensure consistent schema handling across platforms.
...
\begin{lstlisting}[style=mystyle, caption=Contract-Scoped Functionality Pattern]
...
\begin{lstlisting}[language=SQL, style=mystyle, caption=SQL Query Optimization Example]
...
\begin{lstlisting}[language=Python, style=mystyle, caption=Envisioned ML-Ontology Integration]
...
Project timeline constraints significantly influenced scope decisions. The short development window required prioritizing core OBDA functionality over advanced features. Temporal reasoning implementation alone would have consumed 1-2 weeks, leaving insufficient time for comprehensive testing and optimization. This led to focusing on current-state queries rather than historical analysis capabilities.
...
Future development roadmap prioritizes incremental enhancement over revolutionary changes. Phase 1 focuses on temporal reasoning implementation, Phase 2 addresses federation capabilities, and Phase 3 introduces ML integration. This staged approach enables progressive capability building while maintaining system stability.
...
\begin{lstlisting}[style=axiomstyle, caption=TopPlayer Classification Axioms]
// TopPlayer classification axiom
TopPlayer $\equiv$ Player $\sqcap$ hasMarketValue.{v | v $\geq$ 50000000}

// Goalkeeper specialization  
GoalKeeper $\sqsubseteq$ Player $\sqcap$ hasPosition.{"GK"}

// Youth team subsumption
YouthPlayer $\sqsubseteq$ Player $\sqcap$ hasAge.{a | a < 18}
\end{lstlisting}
...
\begin{lstlisting}[style=axiomstyle, caption=Instance Classification Rules]
// Automatic Player classification
$\exists$hasContract.Team $\sqcap$ $\exists$hasJerseyNumber.Integer $\sqsubseteq$ Player

// Contract-based team affiliation
$\exists$hasContract.SeniorTeam $\sqsubseteq$ SeniorPlayer
\end{lstlisting}
...
\begin{lstlisting}[style=axiomstyle, caption=Property Chain Axioms]
// League participation chain
playsFor $\circ$ competesIn $\sqsubseteq$ participatesIn

// Club legend derivation
hasContract $\circ$ hasClubHistory $\circ$ hasLegendStatus $\sqsubseteq$ hasClubLegend

// Coach authority chain
coaches $\circ$ belongsTo $\sqsubseteq$ hasAuthority
\end{lstlisting}

\textbf{General Class Axiom Applications:}
Business rule enforcement through GCAs validates domain constraints:

\begin{lstlisting}[style=axiomstyle, caption=General Class Axioms]
// Role exclusion constraint
Player $\sqcap$ Coach $\sqsubseteq$ $\bot$

// Functional jersey number per team
$\exists$hasJerseyNumber.Integer $\sqcap$ $\exists$playsFor.Team $\sqsubseteq$ $\leq$1 hasJerseyNumber.Integer

// Team hierarchy consistency
YouthTeam $\sqsubseteq$ $\neg$SeniorTeam
\end{lstlisting}
...
\midrule
HermiT 1.4.6 & 3.8 seconds & 1.2s & Development validation \\
Pellet 2.3 & 2.1 seconds & 850ms & Debugging support \\
...
\begin{lstlisting}[style=sparqlstyle, caption=TopPlayer Classification Query]
...
\begin{lstlisting}[language={}, caption=Inference Results Example, basicstyle=\ttfamily\scriptsize, commentstyle=\color{green!40!black}]
=== INFERENCE RESULTS ===
...
[1] TopPlayer Classification:
   ...
   Axiom:  TopPlayer equiv Player and hasMarketValue.\{v | v $\geq$ 50000000\}
...
\begin{lstlisting}[style=sparqlstyle, caption=R2RML Player-Team Mapping Example]
...
:player-team-mapping a rr:TriplesMap ;
    ...
    """ ] ;
    rr:subjectMap [ 
        rr:template "data:player_{person_id}" 
    ] ;
    rr:predicateObjectMap [
        rr:predicate sports:playsFor ;
        rr:objectMap [ 
            rr:template "data:team_{team_id}" 
        ]
    ] .
\end{lstlisting}
...
\textbf{Advanced Constructor Analysis:} Testing reveals significant differences in DL constructor support across reasoning engines. ObjectIntersectionOf ($\sqcap$) constructors enable automatic player classification but generate 28+ OWL 2 QL warnings in Ontop. HermiT provides complete reasoning for complex intersections (TopPlayer $\equiv$ Player $\sqcap$ $\exists$hasMarketValue.$\geq$100M) while SQL engines fail on semantic constraints. Property chains (playsFor $\circ$ competesIn $\rightarrow$ participatesIn) demonstrate 50\% inference coverage in HermiT vs 0\% in SPARQL/Ontop due to OWL 2 QL limitations. This analysis confirms the fundamental trade-off between expressiveness and tractability in practical OBDA systems.
...
\begin{lstlisting}[language={}, caption=Property Chain Semantics, commentstyle=\color{green!40!black}]
...
\begin{lstlisting}[style=axiomstyle, caption=Contract Validity Constraints]
// Contract validity constraint:
ValidContract $\equiv$ $\exists$hasStartDate.Date $\sqcap$ $\exists$hasEndDate.Date $\sqcap$
...
\begin{lstlisting}[style=axiomstyle, caption=Temporal Constraint Axioms]
// Contract validity axiom:
$\forall$hasContract.($\exists$hasStartDate.Date $\sqcap$ $\exists$hasEndDate.Date) $\sqsubseteq$ ValidContract

// No overlapping active contracts:
$\exists$hasContract.Team $\sqcap$ $\exists$hasContract.Team $\sqsubseteq$ $\bot$
...
\begin{lstlisting}[style=axiomstyle, caption=Position Compatibility GCA Examples]
# Position Compatibility Rules
DisjointClasses: Goalkeeper, FieldPlayer
EquivalentClasses: FieldPlayer, (Defender $\sqcup$ Midfielder $\sqcup$ Forward)

# Contract Overlap Prevention
Player $\sqcap$ $\exists$hasActiveContract.Team $\sqcap$ $\exists$hasActiveContract.Team 
    $\sqsubseteq$ $\perp$
    # (where teams are different and periods overlap)

# Performance Threshold Validation  
TopPerformer $\equiv$ Player $\sqcap$ $\exists$hasGoals.$\{g | g > 20\}$ $\sqcap$
...
\begin{lstlisting}[language=Java, style=mystyle, caption=Hybrid Data Access Pattern Example]
...
\begin{lstlisting}[style=mystyle, caption=Initial Testing Architecture (Complex)]
...
\begin{figure}[h]
\centering
% TODO: Add development workflow diagram showing Protégé $\rightarrow$ H2 $\rightarrow$ Maven pipeline
\fbox{\parbox{0.8\textwidth}{\centering \vspace{2cm} \textit{[Development Workflow]} \\ \textit{Protégé design $\rightarrow$ Reasoner validation $\rightarrow$ H2 integration $\rightarrow$ Maven automation} \\ \textit{Diagram showing complete development pipeline} \vspace{2cm}}}
...
\textbf{Reasoning Test Enablement:} The seed data specifically enables validation of key reasoning capabilities including automatic TopPlayer classification (market value $\geq$ €100M yields 5 expected instances), YoungPlayer identification (age $<$ 23 produces 4 expected classifications), property chain inference (team $\rightarrow$ league participation generates 8 derived relationships), and inverse property consistency (\texttt{playsFor} $\leftrightarrow$ \texttt{hasPlayer} bidirectional validation across 12 player-team associations).
...
\begin{lstlisting}[language=Java, style=mystyle, caption=Example Consistency Test Pattern]
...
\begin{lstlisting}[style=sparqlstyle, caption=TopPlayer Classification Test Query]
...
# Test Result: 100% accuracy, 3/3 correct classifications
\end{lstlisting>
...
\begin{lstlisting}[style=mystyle, caption=Performance Benchmarking Results]
...
\begin{lstlisting}[style=mystyle, caption=Maven Project Structure]
sport-ontology/
...
\begin{lstlisting}[language=bash, style=mystyle, caption=Quick Start Installation Example]
...
\begin{lstlisting}[style=sparqlstyle, caption=Talent Scouting Query Example]
# Talent Scouting Query: Find young players in Premier League
# with market value <= 20M in Forward/Midfielder positions
...
\begin{itemize}
    \item Mbappe: PSG jersey \#7 (2017-2024) $\land$ Real Madrid jersey \#10 (2024-present)
...
\begin{lstlisting}[language={}, caption=Optimized NULL-safe R2RML Mapping, commentstyle=\color{green!40!black}]
...
\begin{itemize}
    \item Query execution time: \textbf{340ms $\rightarrow$ 180ms} (47\% improvement)
    \item Memory usage: \textbf{150MB $\rightarrow$ 85MB} (43\% reduction)
\end{itemize}
...
\begin{lstlisting}[style=mystyle, caption=OWL 2 QL Compliance Warning, basicstyle=\ttfamily\scriptsize]
...
Artificial Intelligence (AI) tools were employed throughout the development of this project to enhance productivity, improve code quality, and assist with documentation. Specifically, AI support was utilized for:

\begin{itemize}
\item \textbf{Code Refactoring and Optimization:} Improving code readability, efficiency, and maintainability while adhering to best software engineering practices.
\item \textbf{Debugging Assistance:} Identifying potential logical or syntactical errors and suggesting corrective actions during the testing and validation phases.
\item \textbf{Documentation Generation:} Drafting and refining sections of technical reports, README files, and user manuals to ensure clarity and completeness.
\item \textbf{Test Development:} Assisting in the creation of comprehensive test cases to validate system performance, reasoning correctness, and consistency across components.
\end{itemize}

While AI contributed as a supportive development aid, all \textbf{core design decisions}, \textbf{architectural definitions}, and \textbf{final implementations} were entirely conceived, structured, and validated by the project authors. AI-generated suggestions were reviewed, adapted, and integrated only after thorough human verification to ensure alignment with the project’s objectives and academic integrity standards.

The authors acknowledge AI as a valuable tool that streamlined development and enhanced documentation consistency, yet emphasize that the intellectual ownership, conceptual framework, and system realization remain original and human-driven.
