...
% Brief overview of the project and domain selection
% Motivation for choosing sports ontology
...
\begin{figure}[h]
\centering
% TODO: Add Protégé class hierarchy screenshot showing Person > Player/Coach hierarchy
% Placeholder for now - image to be added later
\fbox{\parbox{0.8\textwidth}{\centering \vspace{2cm} \textit{[Class Hierarchy Diagram]} \\ \textit{Person → Player/Coach specialization} \\ \textit{Screenshot from Protégé to be inserted} \vspace{2cm}}}
\caption{Core class hierarchy showing Person taxonomy with specialized football roles}
...
Functional property conflicts emerged when modeling temporal sports data. The hasJerseyNumber property exemplified this challenge - players legitimately wear different numbers across career transitions, violating functional semantics. Similar issues affected hasPosition properties when players change roles within teams. These conflicts threatened logical consistency while representing valid real-world scenarios.
...
\end{lstlisting}
SubClassOf: Defender, FieldPlayer  
SubClassOf: Midfielder, FieldPlayer
SubClassOf: Forward, FieldPlayer

Contract Overlap Prevention:
SubClassOf: 
    Player and (hasContract some 
        (Contract and (hasTeam some Team))),
    (hasContract max 1 (Contract and (hasEndDate some xsd:date)))
\end{verbatim}
...
Maven automation eliminated JAR management complexity. The integrated build system provides one-command deployment with \texttt{mvn clean test} executing comprehensive validation across SQL, SPARQL, and reasoning components. This approach supports reproducible testing and simplified project distribution.
...
F. M. S. Silva, ``Sport Ontology OBDA System,'' GitHub, 2025. [Online]. Available: \url{https://github.com/fmssilva/sport\_ontology}. Accessed: Oct. 25, 2025.
